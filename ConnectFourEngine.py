...
class ConnectFour:
    ...
    def win_check(self,fieldheights=None):
        # TODO: implement victory conditions
        if fieldheights == None:
            fieldheights=[self.board.col_height(col) for col in range(self.board.width)]
        red_win = self.score_red >= self.board.winscore
        blue_win = self.score_blue >= self.board.winscore
        full_board = all([(col) == self.board.height for col in fieldheights])
        return red_win or blue_win or full_board

    def set_winner(self):
        self.game_running = False
        if self.red_player != None: #Only calculates move times for AI, not humans
            self.total_red_time = sum(self.red_times)
            self.mean_red_time = self.total_red_time / len(self.red_times)
            print("Red Times: {}".format(self.red_times))
            print("Total Time Spent by Red: {} seconds".format(self.total_red_time/1000))
            print("Mean Time Spent by Red per Move: {} seconds".format(round(self.mean_red_time/1000, 5)))
        if self.blue_player != None: #Only calculates move times for AI, not humans
            self.total_blue_time = sum(self.blue_times)
            self.mean_blue_time = self.total_blue_time / len(self.blue_times)
            print("Blue Times: {}".format(self.blue_times))
            print("Total Time Spent by Blue: {} seconds".format(self.total_blue_time/1000))
            print("Mean Time Spent by Blue per Move: {} seconds".format(round(self.mean_blue_time/1000, 5)))
