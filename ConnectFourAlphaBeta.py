...
def max_play(game, field, fieldheights, remaining_ply, alpha, beta):
    ...
    for col in range(0, game.board.width):
        ...
        if (fieldheights[col] < game.board.height):
            ...
            moves += [(col,value)]
            bestVal = max(moves, key = lambda x: x[1]) #Returns the best move for the player at that step in the tree
...
def min_play(game, field, fieldheights, remaining_ply, alpha, beta):
    ...
    for col in range(0, game.board.width):
        ...
        if (fieldheights[col] < game.board.height):
            ...
            newheights = copy.deepcopy(fieldheights)
            #Insert a new token
            token = 1 if not(game.red_turn) else 2 #Minimizing player plays opposite tokens to maximising player
            newfield[col][newheights[col]] = token
            newheights[col] += 1
            if remaining_ply <= 0:
                value = state_score(game, newfield)
            ...
            moves += [(col,value)]
            bestVal = min(moves, key = lambda x: x[1]) #Returns the best move for the opponent at this step in the tree
...
def AIcheck(game, token):
    ...
    remaining_ply = 4
    print("AlphaBeta Playing...")
    print("Ply: {}".format(remaining_ply))
    fieldheights = [game.board.col_height(col) for col in range(game.board.width)]
    (move, value) = max_play(game, game.board.field, fieldheights, remaining_ply, -math.inf, math.inf)
    return move
